...
def mean(town, strng)
  info = strng.split(/\n/).map { |line| line.split(':') }.select do |sub_arr|
    location, _ = sub_arr
    location == town
  end
  return -1 if info.empty?
   
  _, rainfall = info[0]
  rain_hash = rainfall.split(',').map { |mon| mon.split }.to_h
  rain_hash.values.map(&:to_f).sum / 12.0
end
...
def variance(town, strng)
  ...
  _, rainfall = info[0]
  rain_arr = rainfall.split(',').map do |mon_data|
    mon, rain = mon_data.split
    [mon, rain.to_f]
  end
  rain_hash = rain_arr.to_h

  mean = rain_hash.values.sum / 12.0
  sq_diff = rain_hash.each_with_object([]) do |(_, rain), arr|
    arr << (rain - mean)**2
  end
